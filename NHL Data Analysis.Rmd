...
for (i in 1:38) {
  ...
  if (i == 1) {GoalieRecords <- as.data.frame(json_goalierec)} else {GoalieRecords <- rbind(GoalieRecords, as.data.frame(json_goalierec))}
}

GoalieRecords <- GoalieRecords %>% mutate(Win.Percentage = data.wins / data.gamesPlayed)

g1 <- ggplot(GoalieRecords, aes(x = Win.Percentage, , y = ..density..))
g1 + geom_histogram(color = "white", fill = "#666666", bins = 35) + geom_density(color = "blue") + labs(title = "Histogram for Win Percentage")
...
g2 <- ggplot(GoalieRecords, aes(x = data.gamesPlayed, , y = ..density..))
g2 + geom_histogram(color = "white", fill = "#666666", bins = 35) + geom_density(color = "blue") + labs(title = "Histogram for Games Played")
```
...
g3 <- ggplot(GoalieRecords, aes(x = data.gamesPlayed, y = Win.Percentage, color = Who.Stands.Out))
g3 + geom_point() + labs(title = "Goalie Win Percentage by Career Games Played") + xlab("Games Played") + ylab("Win Percentage") + scale_color_manual(values=c("grey60", "green3", "firebrick3", "darkorange2"))
...
g2 <- ggplot(GoalieRecords, aes(x = data.mostGoalsAgainstOneGame, , y = ..density..))
g2 + geom_histogram(color = "white", fill = "#666666", bins = 35) + labs(title = "Histogram for Single Game Goals Scored Against")
```
...
g2 <- ggplot(ScoredAgainst, aes(x = Most.Scored.Against, y = Win.Percentage, color = Most.Scored.Against))
g2 + geom_boxplot(fill = "white", colour = "black") + labs(title = "Boxplots of Win Percentage by Most Goals Scored Against") + geom_point(position = "jitter")
```
...
This is just because I was curious and wanted to explore more of the data.
...
g1 <- ggplot(RegSeason, aes(x = Playoff.Series, y = Win.Percentage, color = Playoff.Series))
g1 + geom_boxplot(fill = "white", colour = "black") + geom_point(position = "jitter") + labs(title = "Win Percentage by Playoff Outcome, 2014 - Present") + scale_x_discrete(labels = function(Playoff.Series) str_wrap(Playoff.Series, width = 10))
```
