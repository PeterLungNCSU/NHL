...
nhl <- function(table,...) {
  ...
  else if (table == "season_rec") {df <- season_rect(...)}
...
```{r teamtot2, echo=FALSE}
seasrec <- nhl(season_rec(team = "all"))

datatable(seasrec, extensions = c('FixedColumns',"FixedHeader"), options = list(scrollX = FALSE, paging=TRUE, fixedHeader=TRUE))
```
...
```{r tgoalierec2, echo = FALSE, warnings = FALSE, messages = FALSE}
tGoalieRecords <- filter(GoalieRecords, data.franchiseName == team)
...
```{r tgoalierec3, echo = FALSE, warnings = FALSE, messages = FALSE}
...
```{r tgoalierec4, echo = FALSE, warnings = FALSE, messages = FALSE}
#Find out who had the most losses
...
```{r tgoalierec7, echo = FALSE, warnings = FALSE, messages = FALSE}
...
PlayerData <- rbind(select(filter(GoaliePull, data.franchiseName == team), data.firstName, data.lastName), select(SkaterPull, data.firstName, data.lastName))
